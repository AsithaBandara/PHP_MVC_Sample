...
class ManagerModel extends Model {
    ...
    public function listManager($id) {
        $sth = $this->db->prepare("SELECT * FROM camss_manager WHERE id = :id ");
    ...
    public function ManagerList() {
        ...
        $sth = $this->db->prepare("SELECT * FROM camss_manager");
        $sth->execute();
        return $sth->fetchAll();
    }
    ...
    public function create($data) {
        try {
            ...
            if ($sth->rowCount() > 0) {
                ...
                $sth = $this->db->prepare("INSERT INTO camss_manager (`id`,`email`,`description`,`status`,`created_at`) VALUES (:id,:username,:description,:status,:created_at)");
    ...
    public function editSave($data) {
        try {
            ...
            $sth = $this->db->prepare("UPDATE camss_manager SET `email` = :username , `description` = :description,`status`= :status ,`updated_at` = :updated_at WHERE `id` = :id");
    ...
    public function delete($id) {
        ...
        $sth = $this->db->prepare("DELETE FROM camss_manager WHERE id = :id");
        $sth->execute(array(
            ':id' => $id
        ));
    }
